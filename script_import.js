...
// 2. REWRITE processExcel
const newProcessExcel = \  const generatePreview = (mapping: any, data: any[][]) => {
  ...
  const processExcel = async (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // 1. Detección por Títulos (Fase 1)
        ...
        let finalCostIdx = firstRow.findIndex(h => h.includes("costo") || h.includes("compra") || h.includes("adquisicion") || h.includes("unitario") || h.includes("neto"));
        ...
        if (finalNameIdx === -1) {
           ...
           if (finalNameIdx !== -1) source.name = "🧠 IA (Textos Largos)";
        }
...
const oldProcessExcelStart = code.indexOf('const processExcel = async (file: File) => {');
const oldProcessExcelEnd = code.indexOf('const processAIFile = async (file: File) => {');
code = code.substring(0, oldProcessExcelStart) + newProcessExcel + '\n\n  ' + code.substring(oldProcessExcelEnd);
