...
export default function InventoryModule() {
  ...
  const avgMargin = items.length > 0 ? items.reduce((acc, i) => acc + ((i.price - i.cost) / i.cost), 0) / items.length : 0.5;
  ...
  const undoLastImport = () => {
    alert("🔙 La función de deshacer temporalmente deshabilitada por seguridad al migrar a la nube Supabase.");
  };
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px', height: '100%' }}>
      <div className="glass-panel flex-between" style={{ padding: '20px' }}>
        <div>
          <h3 style={{ margin: 0, color: 'var(--color-primary)' }}>Módulo de Almacén (Supabase Cloud ☁️)</h3>
      ...
      <div className="glass-panel" style={{ flex: 1, overflowY: 'auto', padding: '0' }}>
        {isLoading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--color-secondary)' }}>☁️ Sincronizando datos con Supabase...</div>
        ) : (
                ...
                return (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--glass-border)', background: rowBg }}>
                    ...
                    <td style={{ padding: '15px', fontWeight: 'bold' }}>
                      {item.name}
                      {item.priceChanged === 'up' && <span style={{ marginLeft: '10px', color: '#ef4444', fontSize: '0.8rem' }}>⚠️ INFLACIÓN</span>}
                      <div style={{ fontSize: '0.75rem', color: 'var(--color-secondary)' }}>Prov: {item.supplier || 'N/A'}</div>
                    </td>
                    <td style={{ padding: '15px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ background: 'rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--color-secondary)' }}>📍 {item.location || 'PENDIENTE'}</span>
                        {item.location && <button onClick={() => printDualLabel(item.location!, item.name, item.code || item.id)} title="Imprimir Etiqueta" className="btn-primary" style={{ padding: '4px 8px', fontSize: '0.9rem' }}>🖨️</button>}
                      </div>
                    </td>
                    <td style={{ padding: '15px', fontWeight: 'bold' }}>{item.stock}</td>
                    <td style={{ padding: '15px' }}>${Number(item.cost).toFixed(2)}</td>
                    <td style={{ padding: '15px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: item.autoPriced ? 'var(--color-secondary)' : 'white', fontWeight: 'bold' }}>${Number(item.price).toFixed(2)}</span>
                      </div>
                    </td>
                  </tr>
                )
        ...
        <SmartImporter 
          ...
          onImport={async (newProducts) => {
            ...
            for (const p of newProducts) {
              const existing = items.find(i => i.code === p.code && i.code !== "");
              if (existing) {
                const inflationFlag = p.cost > existing.cost ? 'up' : null;
                await supabase.from('inventory').update({
                  cost: p.cost, price: p.price, stock: existing.stock + p.stock, supplier: p.supplier || existing.supplier, priceChanged: inflationFlag
                }).eq('id', existing.id);
                updatedCount++;
              } else {
                await supabase.from('inventory').insert({
                  code: p.code, name: p.name, cost: p.cost, price: p.price, stock: p.stock, minStock: p.minStock, location: p.location, supplier: p.supplier, autoPriced: true
                });
                newCount++;
              }
            }
            await fetchInventory();
            alert(`✅ ERIKA Procesó la Importación en la NUBE.\n\n📊 Actualizados: ${updatedCount} productos.\n🆕 Nuevos: ${newCount} productos.`);
          }} 
        />
      )}
    </div>
