...
interface Denominations {
  ...
  m10: number; m5: number; m2: number; m1: number; m05: number;
}
...
export default function CajaModule() {
  ...
  const fetchSession = async () => {
    setIsLoading(true);
    const { data: activeSession } = await supabase.from('cash_sessions').select('*').eq('status', 'open').order('opened_at', { ascending: false }).limit(1).single();
    if (activeSession) {
      setSession(activeSession);
      const { data: txs } = await supabase.from('cash_transactions').select('*').eq('session_id', activeSession.id).order('created_at', { ascending: false });
      setTransactions(txs || []);
    } else {
      setSession(null);
      setTransactions([]);
  ...
  const registerMovement = async (type: 'deposit' | 'withdrawal') => {
    const amountStr = window.prompt(`Monto del ${type === 'deposit' ? 'Ingreso' : 'Retiro'}:`);
    if (!amountStr) return;
  ...
  const calculateTotalCounted = () => {
    return (denoms.b1000 * 1000) + (denoms.b500 * 500) + (denoms.b200 * 200) + (denoms.b100 * 100) + 
           (denoms.b50 * 50) + (denoms.b20 * 20) + (denoms.m10 * 10) + (denoms.m5 * 5) + 
           (denoms.m2 * 2) + (denoms.m1 * 1) + (denoms.m05 * 0.5);
  };

  const closeRegister = async () => {
    const pass = window.prompt("🔒 Contraseña de Gerente para Autorizar Corte:");
    if (pass !== "admin123") return alert("Acceso Denegado.");

    const expectedSales = transactions.filter(t => t.type === 'sale').reduce((sum, t) => sum + t.amount, 0);
    const expectedDeposits = transactions.filter(t => t.type === 'deposit').reduce((sum, t) => sum + t.amount, 0);
    const expectedWithdrawals = transactions.filter(t => t.type === 'withdrawal').reduce((sum, t) => sum + t.amount, 0);
    
    const expectedTotal = Number(session.initial_balance) + expectedSales + expectedDeposits - expectedWithdrawals;
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '20px', position: 'relative' }}>
      ...
      {!session ? (
        <div className="glass-panel flex-center" style={{ flexDirection: 'column', padding: '50px' }}>
          <h2 style={{ color: 'var(--color-secondary)' }}>La Caja está Cerrada</h2>
          <p>Para empezar a cobrar, necesitas declarar el fondo inicial (morralla).</p>
          <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
            <span style={{ fontSize: '1.5rem', alignSelf: 'center' }}>$</span>
            <input type="number" value={initialBalance} onChange={e => setInitialBalance(Number(e.target.value))} style={{ padding: '15px', fontSize: '1.2rem', background: 'rgba(0,0,0,0.5)', color: 'white', border: '1px solid var(--color-primary)', borderRadius: '8px' }} />
            <button className="btn-primary" onClick={openRegister}>Abrir Turno</button>
          </div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div className="glass-panel" style={{ background: 'rgba(16, 185, 129, 0.05)' }}>
              <h3>🟢 Caja Abierta</h3>
              <p>Turno iniciado: {new Date(session.opened_at).toLocaleString()}</p>
              <p style={{ fontSize: '1.2rem', color: 'var(--color-secondary)' }}>Fondo Inicial: <strong>${session.initial_balance}</strong></p>
              
              <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                <button className="btn-primary" style={{ background: 'transparent', border: '1px solid #3b82f6', color: '#3b82f6' }} onClick={() => registerMovement('deposit')}>+ Ingresar Efectivo</button>
                <button className="btn-primary" style={{ background: 'transparent', border: '1px solid #ef4444', color: '#ef4444' }} onClick={() => registerMovement('withdrawal')}>- Retirar Efectivo</button>
              </div>
            </div>
              ...
              <ul style={{ listStyle: 'none', padding: 0 }}>
                {transactions.map(t => (
                  <li key={t.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    <span>
                      {t.type === 'sale' ? '🛒 Venta' : t.type === 'deposit' ? '⬇️ Ingreso' : '⬆️ Retiro'} 
                      {t.description && <span style={{ color: 'var(--color-secondary)', marginLeft: '10px', fontSize: '0.8rem' }}>({t.description})</span>}
                    </span>
                    <strong style={{ color: t.type === 'withdrawal' ? '#ef4444' : '#10b981' }}>
          ...
          <div className="glass-panel" style={{ border: '1px solid var(--color-primary)' }}>
            <h2 style={{ color: 'var(--color-primary)', textAlign: 'center' }}>🔒 Corte de Caja Ciego</h2>
            <p style={{ textAlign: 'center', opacity: 0.7, marginBottom: '20px' }}>Cuenta el dinero físico que tienes en el cajón.</p>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div>
                <h4 style={{ color: '#10b981' }}>💵 Billetes (Cantidad)</h4>
                {[1000, 500, 200, 100, 50, 20].map(val => (
                  <div key={val} className="flex-between" style={{ marginBottom: '10px' }}>
                    <span>$ {val}</span>
                    <input type="number" min="0" value={denoms[`b${val}` as keyof Denominations]} onChange={e => setDenoms({...denoms, [`b${val}`]: Number(e.target.value) || 0})} style={{ width: '60px', padding: '5px', background: 'black', color: 'white', border: '1px solid #333' }} />
                  </div>
                ))}
              </div>
              <div>
                ...
                {[10, 5, 2, 1, 0.5].map(val => {
                  ...
                  return (
                    <div key={val} className="flex-between" style={{ marginBottom: '10px' }}>
                      <span>$ {val}</span>
                      <input type="number" min="0" value={denoms[key as keyof Denominations]} onChange={e => setDenoms({...denoms, [key]: Number(e.target.value) || 0})} style={{ width: '60px', padding: '5px', background: 'black', color: 'white', border: '1px solid #333' }} />
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ marginTop: '30px', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.2)', paddingTop: '20px' }}>
              <h3 style={{ color: 'var(--color-secondary)' }}>Total Físico Contado</h3>
              <h1 style={{ fontSize: '3rem', margin: '10px 0' }}>${calculateTotalCounted().toFixed(2)}</h1>
              <button className="btn-primary" style={{ width: '100%', padding: '15px', background: 'linear-gradient(135deg, var(--color-primary), #059669)' }} onClick={closeRegister}>
